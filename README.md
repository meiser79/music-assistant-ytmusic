...
- The year is the year of the *album entry the track was surfaced under*, not of the recording. A 1998 song on a 2025 compilation reads as 2025. Requested in [#53](https://github.com/sproft/music-assistant-ytmusic/issues/53).
...
- Upgrade ytmusicapi inside the Music Assistant container with `pip install --upgrade ytmusicapi`, then **restart Music Assistant**. The restart is load-bearing: the old module is already imported in the running process, and neither the upgrade nor a provider reload replaces it. You have it right when the `get_library_podcasts failed` warning stops appearing and the podcast sync completes.
...
- Until it is upgraded that category fails its sync rather than reporting no subscriptions. That is deliberate: reporting an empty list would make Music Assistant unsubscribe you from every show. Background: [#64](https://github.com/sproft/music-assistant-ytmusic/issues/64).
...
- The same rule now covers a library call that fails outright. A rate limit, a timeout, or a response whose shape YouTube changed used to be written to the log and then answered with an empty list, which Music Assistant read exactly the way it read a lapsed cookie. Any failure in a library call now fails that sync and leaves your library where it was. This includes a failure that only affects half a category: artists are read as two separate requests, and one of them failing used to delete everything the other did not cover.
- What that looks like: the affected sync shows as failed in Music Assistant with the reason attached, nothing is removed in the meantime, and the next scheduled run picks it up. **If a sync keeps failing, do not remove and re-add the provider.** That deletes the library outright, which is the thing this protects you from. A sync failing every time is telling you something real, usually an expired cookie or an ytmusicapi that needs upgrading. Background: [#64](https://github.com/sproft/music-assistant-ytmusic/issues/64).
